...
public:
    ...
    // RecentlyUsed, type=application
    // HighScore, type=application
    // RecentlyCreated, type=application
    // RecentlyUsed, type=document
    // HighScore, type=document
    // RecentlyUsed, type=document, filter:agent
    // HighScore, type=document, filter:agent
    // Recently used contacts
    // Most frequently used contacts
    //
    // Possibly live changes for ALIs that don't gate the data behind a submenu.
    // Grouping things together by analysing the statistics (more advanced - API does not need to cover it).
    // Application Launching Interface (ALI) search history.
    // Metadata for the recently/most used documents, so they can e.g. be grouped by type.
