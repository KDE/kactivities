...
{
    d->id = activityId;
    connect(Manager::activities(), SIGNAL(ActivityStateChanged(const QString &, int)),
            this, SLOT(activityStateChanged(const QString &, int)));

    connect(Manager::activities(), SIGNAL(ActivityChanged(const QString &)),
            this, SLOT(infoChanged(const QString &)));

    connect(Manager::activities(), SIGNAL(ActivityAdded(const QString &)),
            this, SLOT(added(const QString &)));

    connect(Manager::activities(), SIGNAL(ActivityRemoved(const QString &)),
            this, SLOT(removed(const QString &)));

    connect(Manager::activities(), SIGNAL(ActivityStarted(const QString &)),
            this, SLOT(started(const QString &)));

    connect(Manager::activities(), SIGNAL(ActivityStopped(const QString &)),
...
QString Info::name() const
{
    KACTIVITYINFO_DBUS_CAST_RETURN(
        QString, QString, Manager::activities()->ActivityName(d->id));
}

QString Info::icon() const
{
    KACTIVITYINFO_DBUS_CAST_RETURN(
        QString, QString, Manager::activities()->ActivityIcon(d->id));
}
...
{
    KACTIVITYINFO_DBUS_CAST_RETURN(
            QString, QString, Manager::activities()->ActivityName(id));
}
...
bool Info::isEncrypted() const
{
    return false;
    // KACTIVITYINFO_DBUS_CAST_RETURN(
    //         bool, bool, Manager::activities()->IsActivityEncrypted(d->id));
}
