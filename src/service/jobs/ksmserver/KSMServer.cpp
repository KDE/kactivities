        ...
        const QString & servicePath, const QString & path, const QString & object, Func init)
{
    ...
    kDebug() << path << "is valid?" << service->isValid();
...
void KSMServer::Private::startCallFinished(QDBusPendingCallWatcher * call)
{
     ...
     if (reply.isError()) {
         kDebug() << "Session starting call failed, but we are returning success";
         ...
         kDebug() << "Did we start the activity successfully:" << retval;
         if (!retval) {
...
void KSMServer::Private::stopCallFinished(QDBusPendingCallWatcher * call)
{
    ...
    if (reply.isError()) {
        kDebug() << "Session stopping call failed, but we are returning success";
        ...
        kDebug() << "Did we stop the activity successfully:" << retval;
        if (!retval) {
